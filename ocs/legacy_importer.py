"""
Legacy implementation retained behind the refactored automation modules.
...
CATALOG_TIMING_MODES = ("in_camp", "external")
DEFAULT_TIMING_SETTINGS = {
    "morning_cutoff": "12:00",
    "in_camp": {
        "medic": "05:45",
        "ammo_collection": "05:30",
        "transport": "07:00",
        "vehicle": "05:45",
    },
    "external": {
...
def _normalize_timing_mode(value):
    """Return a normalized timing profile name, or None for no timing."""
    ...
    if normalized in {"external", "out_of_camp", "outcamp"}:
        return "external"
    if normalized in {"none", "inactive", "disabled"}:
        return None
    return normalized


def conduct_timing_mode(conduct):
    """Return timing independently from whether the catalogue row is active."""
    if "timing" in conduct:
        return _normalize_timing_mode(conduct.get("timing"))

    # Version 2 stored the timing profile in ``active``. Keep old uploads working.
    if "active" not in conduct or conduct.get("active") is True:
        return "in_camp"
    return _normalize_timing_mode(conduct.get("active"))


def _conduct_is_active(conduct):
    """Return the independent catalogue inclusion switch."""
    if "active" not in conduct:
        return True
    value = conduct.get("active")
    if isinstance(value, bool):
        return value
    if value in (None, ""):
        return False

    # Version 2 used In camp/External as both timing and activation.
    normalized = str(value).strip().casefold().replace("-", "_").replace(" ", "_")
    if normalized in CATALOG_TIMING_MODES:
        return True
    if normalized in {"none", "inactive", "disabled", "false", "no", "0"}:
        return False
    return False


def _migrate_catalog_controls(conduct):
    """Split the legacy combined active/timing value into version 3 fields."""
    timing = conduct_timing_mode(conduct)
    conduct["active"] = _conduct_is_active(conduct)
    conduct["timing"] = timing


def normalize_timing_settings(settings=None):
    """Merge browser/config timing values with safe application defaults."""
    supplied = settings if isinstance(settings, dict) else {}
    normalized = {
        "morning_cutoff": str(
            supplied.get("morning_cutoff", DEFAULT_TIMING_SETTINGS["morning_cutoff"])
            or DEFAULT_TIMING_SETTINGS["morning_cutoff"]
        ).strip(),
    }
    if not PREPARATION_TIME_PATTERN.fullmatch(normalized["morning_cutoff"]):
        normalized["morning_cutoff"] = DEFAULT_TIMING_SETTINGS["morning_cutoff"]

    for mode in CATALOG_TIMING_MODES:
        profile = supplied.get(mode, {})
        if not isinstance(profile, dict):
            profile = {}
        normalized[mode] = {}
        for item_name in PREPARATION_ITEMS:
            default = DEFAULT_TIMING_SETTINGS[mode][item_name]
            value = str(profile.get(item_name, default) or default).strip()
            normalized[mode][item_name] = (
                value if PREPARATION_TIME_PATTERN.fullmatch(value) else default
            )
    return normalized


def _ensure_preparation_fields(conduct):
    _migrate_catalog_controls(conduct)
...
def validate_conduct_catalog(catalog, lesson_plan_names=None):
    ...
    for row_number, conduct in enumerate(catalog.get("conducts", []), start=1):
        ...
        if not isinstance(conduct.get("active"), bool):
            errors.append(
                f"{conduct_id or f'row {row_number}'} active must be true or false."
            )

        timing_mode = conduct.get("timing")
        if timing_mode not in (*CATALOG_TIMING_MODES, None):
            errors.append(
                f"{conduct_id or f'row {row_number}'} timing must be in_camp, external, or blank."
            )
    ...
    for alias, conduct_ids in seen_aliases.items():
        active_ids = {
            ...
            if _conduct_is_active(conduct) and conduct.get("conduct_id") in conduct_ids
        }
        if len(active_ids) > 1:
            errors.append(
...
def build_preparation_schedule(
    ...
    """Calculate preparation windows from the conduct profile and shared presets."""
    ...
    mode = conduct_timing_mode(conduct_rule)
    settings = normalize_timing_settings(timing_settings)
    profile = settings.get(mode, {}) if mode in CATALOG_TIMING_MODES else {}
    flags = resource_flags if isinstance(resource_flags, dict) else {}
    morning_cutoff = datetime.strptime(
        settings["morning_cutoff"],
        "%H:%M",
    ).time()
    is_morning = activity_start is not None and activity_start.time() < morning_cutoff
    at_mmrc = bool(flags.get("mmrc", False))
    schedule = []

    for item_name in PREPARATION_ITEMS:
        item = preparation.get(item_name, {})
        duration = int(item.get("duration_minutes", 0) or 0)
        configured_time = str(profile.get(item_name, "") or "").strip()
        required = bool(flags.get(item_name, duration > 0))

        if item_name == "medic":
            required = required and is_morning
        elif item_name == "ammo_collection":
            required = required and not at_mmrc
        elif item_name == "vehicle":
            if resource_flags is not None:
                has_profile_vehicle = (
                    bool(flags.get("luv_hq", False))
                    if mode == "external"
                    else bool(flags.get("souv", False)) and is_morning
                )
                required = required and has_profile_vehicle
            required = required and not at_mmrc

        if mode not in CATALOG_TIMING_MODES:
            scheduled_start = None
            scheduled_end = None
        elif required and configured_time:
            scheduled_start = datetime.combine(
                activity_day,
                datetime.strptime(configured_time, "%H:%M").time(),
            )
            scheduled_end = scheduled_start + timedelta(minutes=duration)
        elif required and activity_start is not None:
...
def match_catalog_conduct(conduct_text, catalog, lesson_name_index):
    ...
    if legacy_clean in lesson_values:
        ...
        if exact_rule and not _conduct_is_active(exact_rule):
            return {
                "status": "inactive",
                "target": legacy_clean,
                "candidates": [],
    ...
    active_rules = [
        rule for rule in catalog_rules if _conduct_is_active(rule)
    ]
    inactive_rules = [
        rule for rule in catalog_rules if not _conduct_is_active(rule)
    ]
    exact_candidates = []
    contains_candidates = []
    ...
    def draft_siao(self, cadet_size: int, output_path=None):
        ...
        for c in conduct_mapping:
            for match_name, display_name in zip(
                ...
                ammo_required = any(
                    source_column < self.lesson_plan.shape[1]
                    and _has_siao_allocation(
                        self.lesson_plan.iloc[row_num, source_column]
                    )
                    for source_column in SIAO_MAPPING_ARMS.values()
                )
                luv_hq_column = vehicle_columns.get("LUV (HQ)")
                souv_column = vehicle_columns.get("SOUV")
                luv_hq_required = (
                    luv_hq_column is not None
                    and _has_siao_allocation(
                        self.lesson_plan.iloc[row_num, luv_hq_column]
                    )
                )
                souv_required = (
                    souv_column is not None
                    and _has_siao_allocation(
                        self.lesson_plan.iloc[row_num, souv_column]
                    )
                )
                venue_columns = [3]
                venue_columns.extend(
                    column
                    for key in (
                        "Reporting Venue",
                        "Destination Venue",
                        "Military Transport Venue From",
                        "Military Transport Venue To",
                    )
                    if (column := vehicle_columns.get(key)) is not None
                )
                at_mmrc = any(
                    "MMRC" in normalize_conduct_name(
                        self.lesson_plan.iloc[row_num, column]
                    )
                    for column in venue_columns
                    if column < self.lesson_plan.shape[1]
                )
                resource_flags = {
                    "medic": medic == "Y",
                    "ammo_collection": ammo_required,
                    "transport": bool(rule and rule.get("bus_required", False)),
                    "vehicle": luv_hq_required or souv_required,
                    "luv_hq": luv_hq_required,
                    "souv": souv_required,
                    "mmrc": at_mmrc,
                }
                ...
                parkover_vehicles = []
                if five_ton_column is not None and _has_siao_allocation(
                    ws.cell(row=start_row, column=five_ton_column).value
                ):
                    parkover_vehicles.append("5T")
                if souv_required and conduct_timing_mode(rule or {}) == "external":
                    parkover_vehicles.append("SOUV")
                ...
                if is_xaw_recce and parkover_column is not None:
                    ...
                    parkover_column is not None
                    and parkover_vehicles
                ):
                    ...
                    ws.cell(
                        row=start_row,
                        column=parkover_column,
                        value=f"{' + '.join(parkover_vehicles)} PARKOVER ON {parkover_day.strftime('%d%m%y')}",
                    )
